...
void CreatedHealthFlags() {
    ...
    GeneralCapabilityLog({.message = "File has Linux capabilities assigned", .id = ID("CAP01"), .level = 3});
    ...
    GeneralCapabilityLog({.message = "Capability-enabled binary found outside standard system directories", .id = ID("CAP04"), .level = 5});
    GeneralCapabilityLog({.message = "File with capabilities not owned by root", .id = ID("CAP05"), .level = 4});
    ...
    GeneralCapabilityLog({.message = "Linux capabilities set on a symbolic link", .id = ID("CAP09"), .level = 2});
    GeneralCapabilityLog({.message = "Linux capabilities set on a directory", .id = ID("CAP10"), .level = 2});
    ...
    GeneralCapabilityLog({.message = "Redundant configurations: Binary has both SUID/SGID and capabilities", .id = ID("CAP19"), .level = 2});
    ...
    GeneralCapabilityLog({.message = "Capability-enabled binary also carries the sticky bit", .id = ID("CAP22"), .level = 2});
    ...
    GeneralCapabilityLog({.message = "Identity manipulation capability assigned (CAP_SETUID/CAP_SETGID/CAP_CHOWN)", .id = ID("CAP26"), .level = 4});
