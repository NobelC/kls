...
namespace kls::platform{
  [[nodiscard]] std::string_view IdentityResolver::owners_name(uid_t uid){
    ...
    int s_err;
    while((s_err = getpwuid_r(uid,&pwd,buffer.data(), bufflen,&result_owner)) == ERANGE){
  ...
  [[nodiscard]] std::string_view IdentityResolver::groups_name(gid_t gid){
    ...
    int s_err;
    while((s_err = getgrgid_r(gid,&gp,buffer.data(),bufflen,&result_group)) == ERANGE ){
      buffer.resize(buffer.size() * 2);
    }
