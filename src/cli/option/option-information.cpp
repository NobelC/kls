...
void CreatedOptionData() {
  ...
  no_color.handler = std::monostate{};
  ...
  explain_code.handler = std::monostate{}; 
  ...
  only_capabilities.handler = std::monostate{};
  ...
  filter.handler = FilteringProcess([](FilterStruct &filter_contex) {
  ...
  modified_before.handler = FilteringProcess([](FilterStruct &filter_contex) {
    const auto *date_ptr =
        std::any_cast<std::string_view>(&filter_contex.context);
    if (!date_ptr) {
      return;
  ...
  modified_after.handler = FilteringProcess([](FilterStruct &filter_contex) {
    const auto *date_ptr =
        std::any_cast<std::string_view>(&filter_contex.context);
    if (!date_ptr) {
      return;
  ...
  extension.handler = FilteringProcess([](FilterStruct &filter_contex) {
  ...
  larger_than.handler = FilteringProcess([](FilterStruct &filter_contex) {
    const auto *size_raw =
        std::any_cast<std::string_view>(&filter_contex.context);
    if (!size_raw) {
      return;
  ...
  smaller_than.handler = FilteringProcess([](FilterStruct &filter_contex) {
    const auto *size_raw =
        std::any_cast<std::string_view>(&filter_contex.context);
    if (!size_raw) {
      return;
...
owner_filter.handler = FilteringProcess([](FilterStruct &filter_contex) {
...
GeneralOptionLog(owner_filter);
  ...
  group_filter.handler = FilteringProcess([](FilterStruct &filter_contex) {
  ...
  min_links.handler = FilteringProcess([](FilterStruct &filter_contex){
  ...
  min_severity.handler = FilteringProcess([](FilterStruct &filter_contex){
  ...
  sort.handler = FilteringProcess([](FilterStruct &filter_contex) {
  ...
  reverse.handler = FilteringProcess([](FilterStruct &filter_contex) {
