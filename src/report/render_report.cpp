...
std::string_view conversion_type_file(const kls::filesystem::FileType &type) {
  switch (type) {
...
std::string alerts_pack(const std::vector<ID> &issues) {
  std::string alerts_unificated;
  for (const auto &id : issues) {
    auto finding = kls::findings::get_finding(id);
    auto value_finding = finding->id.get_value();
    auto value_str_finding = finding->id.get_value_unpack();
    alerts_unificated.append(value_str_finding + " | ");
  }
  return alerts_unificated;
}

namespace kls::report {

void render_report(std::ostream &output,
                   const kls::scanner::ScanOutput &entries,
                   const kls::report::RenderOptions &option_render) {
  ...
  if (option_render.show_headers) {
    output << std::format(
        "{:<5} {:<10} {:<3} {:<8} {:<8} {:<10} {:<12} {:<30} {:<30}\n", "TYPE",
        "PERMS", "LNK", "OWNER", "GROUP", "SIZE", "MODIFIED", "NAME", "ALERTS");
  }
  ...
  for (size_t i = 0; i < entries.items.size(); i++) {
    ...
    std::string display_name = actual_entry.name;
    ...
    auto pading_size =
        maximum_name_width - static_cast<unsigned int>(display_name.size());
    ...
    output << std::format(
        "{:<5} {:<10} {:<3} {:<8} {:<8} {:<10} {:<12} {:<30} {:<30} \n", type,
        perms, actual_entry.nlinks, Identity.owners_name(actual_entry.uid),
        Identity.groups_name(actual_entry.gid), size_str,
        std::string_view(str_time.data(), str_time.size()), formatted_name,
        alerts_pack(entries.items[i].findings));
