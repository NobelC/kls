...
int main(int argc, char *argv[]) {
  ...
  if (auto *failure =
          std::get_if<kls::Failure<kls::cli::model::CliError>>(&parse_result)) {
  ...
  auto opts =
      std::get<kls::Success<kls::cli::model::ParsedOptions>>(parse_result)
          .value;

  // 2. Validate
  auto validate_result = kls::cli::validator::validate(opts);
  if (auto *failure = std::get_if<kls::Failure<kls::cli::model::CliError>>(
          &validate_result)) {
    std::cerr << "Error: " << failure->error.message << "\n";
  ...
  opts = std::get<kls::Success<kls::cli::model::ParsedOptions>>(validate_result)
             .value;
  ...
  auto scan_result =
      kls::auditor::audit_orchestrator(opts.target_path, scan_opts);
  if (auto *failure =
          std::get_if<kls::Failure<kls::scanner::ScanError>>(&scan_result)) {
  ...
  auto scan_output =
      std::get<kls::Success<kls::scanner::ScanOutput>>(scan_result).value;
  auto severity_counts =
      kls::cli::adapter::count_findings_by_severity(scan_output);
