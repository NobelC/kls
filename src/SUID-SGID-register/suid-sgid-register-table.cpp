...
void CreatedHealthFlags() {
    ...
    GeneralHealthFlagsLog({.message = "SUID bit set", .id = ID("SU01"), .level = 3});
    ...
    GeneralHealthFlagsLog({.message = "SUID binary is world-executable", .id = ID("SU07"), .level = 4});
    GeneralHealthFlagsLog({.message = "SUID binary is group-writable", .id = ID("SU08"), .level = 4});
    GeneralHealthFlagsLog({.message = "SUID bit set on a symbolic link", .id = ID("SU09"), .level = 2});
    GeneralHealthFlagsLog({.message = "SUID bit set on a directory", .id = ID("SU10"), .level = 2});
    GeneralHealthFlagsLog({.message = "SUID binary owned by unknown UID", .id = ID("SU11"), .level = 4});
    GeneralHealthFlagsLog({.message = "SUID binary recently modified", .id = ID("SU12"), .level = 4});
    GeneralHealthFlagsLog({.message = "SUID binary future timestamp", .id = ID("SU13"), .level = 5});
    GeneralHealthFlagsLog({.message = "SUID binary shows timestamp manipulation", .id = ID("SU14"), .level = 5});
    GeneralHealthFlagsLog({.message = "SUID binary world-readable in sensitive path", .id = ID("SU15"), .level = 3});
    GeneralHealthFlagsLog({.message = "SUID binary ELF magic mismatch", .id = ID("SU16"), .level = 5});
    GeneralHealthFlagsLog({.message = "SUID binary hardlink count warning", .id = ID("SU17"), .level = 3});
    GeneralHealthFlagsLog({.message = "SUID binary not executable but readable", .id = ID("SU18"), .level = 2});
    GeneralHealthFlagsLog({.message = "SUID binary with redundant capabilities", .id = ID("SU19"), .level = 2});
